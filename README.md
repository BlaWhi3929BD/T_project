...
|---|---:|---:|---:|
| `GET /api/stats` | < 200 мс | **232.6 мс** | p95 **460 мс**, max 494 мс |
| `GET /api/trades` | < 100 мс | Не измерялся отдельно | p95 **71 мс**, max 99 мс |

> **Примечание по производительности `/api/stats`:**
> Дневные суммы и все базовые агрегаты вычисляются одним SQL `GROUP BY`, после
> чего Python строит только непрерывную equity curve по дневным строкам.
> Backend запускается с двумя Uvicorn workers. В текущем локальном окружении
> измерения после прогрева не укладываются в заявленный SLA: одиночный запрос
> занял 232.6 мс, а p95 при пяти параллельных запросах составил 460 мс.
> Результат зависит от СУБД, CPU и ресурсов контейнера; это ограничение явно
> сохраняется в документации, а не скрывается изменением методики.
...
# Замер /api/stats:
ab -n 100 -c 5 http://127.0.0.1:8080/api/stats

# Замер /api/trades:
ab -n 100 -c 5 "http://127.0.0.1:8080/api/trades?page=1&page_size=50"

# Отдельный замер одиночного запроса:
ab -n 1 -c 1 http://127.0.0.1:8080/api/stats
```
